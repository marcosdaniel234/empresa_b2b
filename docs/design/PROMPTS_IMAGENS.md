...
- **Cores** (paleta Atlantic Industrial): fundo azul-atlântico profundo
  `#0B1D2A`; destaques em verde-mar `#7FCFC6` / `#4FB3AA`; tons de aço
  `#22506F`. Nada de vermelho, laranja, verde saturado ou neon.
- **Espaço para texto**: os títulos ficam à esquerda. Deixe os **45% da
  esquerda quase vazios** e escuros; o assunto vai à direita.
- **Bordas**: as quatro bordas devem terminar em azul profundo liso, para a imagem
...
## 1. Brasil em fios de luz verde-mar — fundo das faixas e do rodapé
...
sea-glass teal fiber-optic strands and tiny teal beads (#7FCFC6), some
strands arcing between cities like logistics routes, placed on the right
side of the frame, deep atlantic navy background (#0B1D2A), left 45% of the image empty and dark, soft
cool rim light, shallow depth of field, elegant, premium, minimal,
low contrast, 16:9 --ar 16:9 --style raw
```

## 2. Planta técnica dos ativos — faixas internas (Lojas, Leilões, Ajuda)

Desenho técnico de máquinas, como uma planta de engenharia (blueprint). Conversa
direto com o que o site vende.

```
Technical blueprint line drawing of industrial assets — a forklift, a
semi-truck cab, a diesel generator and a CNC lathe — drawn as fine teal
(#7FCFC6) engineering linework with measurement lines and callout circles,
exploded-view style, on a flat deep atlantic navy background (#0B1D2A), drawings
...
of deep atlantic navy (#0B1D2A) and sea-glass teal (#7FCFC6), very dark overall,
cinematic, the light trails flowing from the left edge toward the right,
left 40% nearly black, no text, 16:9 --ar 16:9
```

## 4. Aço naval escovado — textura curta (menu do celular, rodapé)

Textura abstrata, sem assunto, para superfícies pequenas.

```
Seamless abstract texture of brushed dark navy-painted ship steel with a
faint sea-glass teal patina, fine horizontal brushing marks, subtle weld
seams and rivets, very dark,
...
| --- | --- | --- |
| Foto de trabalhador no galpão | Abertura de `/anunciar` (hoje usa a foto institucional) | `Brazilian warehouse supervisor in a navy work shirt checking a tablet next to palletized industrial equipment, natural light from a high window, cool daylight with a steel-blue grade, shallow depth of field, editorial photography, subject on the right, 16:9` |
| Fachadas das sedes | Capa de cada loja em `/loja/[empresa]` | `Exterior of a mid-size Brazilian industrial company building, clean concrete and metal facade, loading docks, overcast coastal light, steel-blue grade, no signage, no logos, architectural photography, 16:9` |
| Pátio de leilão | Faixas de Leilões | `Rows of used industrial machinery and trucks parked in an open auction yard at blue hour near a port, shot from a low angle, cool steel-blue light, dark navy sky, no people, no logos, 16:9` |

## Como me entregar

1. Salve os arquivos em `assets-src/fundos/` (ou `assets-src/institutional/`
   para fotos), com nomes curtos: `brasil-fibra.png`, `planta-ativos.png`…
