...
import { formatCurrencyCard, formatDeadlineLabel } from "@/lib/format";
...
export function AssetCard({ asset, index = 0 }: { asset: Asset; index?: number }) {
  ...
  const isClosed = ["encerrado_vencedor", "encerrado_sem_vencedor", "cancelado"].includes(asset.status);
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-card border border-border-subtle bg-surface-card shadow-card transition-shadow duration-quick hover:shadow-elevated">
      <div className="relative">
        ...
        <FavoriteButton slug={asset.slug} title={asset.title} className="absolute right-3 top-3" />
      </div>
      ...
      <div className="flex flex-1 flex-col gap-1 p-4">
        ...
        <h3 className="text-title-card text-text-primary">
          <Link href={`/leilao/${asset.slug}`} className="static after:absolute after:inset-0 after:content-['']">
            {asset.title}
          </Link>
        </h3>
        {company && <p className="text-metadata text-text-secondary">{company.name}</p>}
        ...
        <div className="mt-3 flex items-end justify-between gap-2 border-t border-border-subtle pt-3">
          <div>
            ...
            <span className="text-title-card font-bold text-text-primary tabular">
              {formatCurrencyCard(hasBid ? (asset.currentBid as number) : asset.startingBid)}
            </span>
          </div>
          {!isClosed && (
            <span className="flex items-center gap-1 text-caption text-text-secondary">
              <Clock className="h-4 w-4" aria-hidden="true" />
              {formatDeadlineLabel(asset.deadlineIso)}
            </span>
          )}
        </div>
        ...
        <Link
          ...
          className="static relative z-10 mt-3 inline-flex h-10 items-center justify-center rounded-control border border-action text-label font-medium text-action transition-colors duration-quick hover:bg-surface-subtle"
        >
          Ver leilão
        </Link>
      </div>
